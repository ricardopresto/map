...
export default {
  ...
  data() {
    return {
      access_token:
        "pk.eyJ1IjoicmljYXJkb3ByZXN0byIsImEiOiJja3J1bmloMHEwYnY4MnBzNWt6b2x6eHBuIn0.zivT6cbQcLcAFl1Q8GW-QA",
  ...
  methods: {
    ...
    adjustCentre() {
      ...
      locationXY = {
        x: locationXY.x - this.mapWidthOffset,
        y: locationXY.y - this.mapHeightOffset
      };
    ...
    dropMarker(e) {
      ...
      let icon = document.createElement("img");
      let dropLocation = {
        x: e.clientX - this.mapWidthOffset,
        y: e.clientY - this.mapHeightOffset
      };
    ...
    addTextMarker(textInfo) {
      ...
      textMarker.id = name;
      textMarker.onclick = e => {
        e.preventDefault();
      };
      this[name] = new mapboxgl.Marker({
        element: textMarker,
        draggable: true
      });
      this[name].setLngLat(this.adjustCentre());
